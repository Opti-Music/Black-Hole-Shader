...
// Psych Engine equivalent uniforms
uniform float iTime;
uniform vec3 iResolution;

#define AA 1   // Keep at 1 for performance in FNF
#define _Speed 3.0  
#define _Steps 12. 
#define _Size 0.3 
...
vec4 background(vec3 ray)
{
    ...
    // Default Psych engine background fallback wrapper since iChannel0 is missing
    vec4 nebulae = flixel_texture2D(bitmap, (uv * 1.5));
...
vec4 raymarchDisk(vec3 ray, vec3 zeroPos)
{
    ...
    {                     
        ...
        float lengthPosInner = length(position.xz);
        float distMult = 1.;

        distMult *= clamp((lengthPosInner - _Size * 0.75) * (1./_Size) * 1.5, 0., 1.);        
        distMult *= clamp((_Size * 10. - lengthPosInner) * (1./_Size) * 0.20, 0., 1.);
        distMult *= distMult;

        float u = lengthPosInner + iTime * _Size * 0.3 + intensity * _Size * 0.2;
        ...
        lengthPosInner *= (1./_Size);
        o.rgb += redShift * (intensity * 1. + 0.5) * (1./_Steps) * 100. * distMult / (lengthPosInner * lengthPosInner);
    }  
...
void main()
{
    // Convert OpenFL coordinates to Shadertoy format
    ...
    for(int j = 0; j < AA; j++)
    {
        for(int i = 0; i < AA; i++)
        {
            ...
            vec3 pos = vec3(0., 0.05, -10.0); // Hardcoded standard distance vector since iMouse is absent
